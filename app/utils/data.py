...
def get_transport():
    ...
    mapping = {"tr_type": "type", "tr_from": "from", "tr_to": "to"}
...
def get_meta(category):
    categories = {
        "tested": {"url": TESTED_CSV_PATH, "start_date": "2020-03-12"},
        "tested_lab": {"url": TESTED_LAB_CSV_PATH, "start_date": "2020-02-24"},
        "confirmed": {"url": CONFIRMED_CSV_PATH, "start_date": "2020-02-21"},
        "dead": {"url": DEAD_CSV_PATH, "start_date": "2020-03-10"},
        "hospitalized": {"url": HOSPITALIZED_CSV_PATH, "start_date": "2020-03-08"},
        "vaccine_doses": {"url": VACCINE_DOSES_CSV_PATH, "start_date": "2020-12-27"},
    }
    ...
    try:
        ...
        if category == "hospitalized":
            ...
            df = df.rename(columns={"new_doses": "new", "total_doses": "total"})
        else:
...
def get_timeseries_new():
    df = pd.DataFrame(index=pd.date_range("2020-02-21", date.today()))

    tested = pd.read_csv(
        TESTED_CSV_PATH, index_col=["date"], usecols=["date", "new"]
    ).rename(columns={"new": "tested"})

    confirmed = pd.read_csv(
        CONFIRMED_CSV_PATH, index_col=["date"], usecols=["date", "new"]
    ).rename(columns={"new": "confirmed"})

    dead = pd.read_csv(
        DEAD_CSV_PATH, index_col=["date"], usecols=["date", "new"]
    ).rename(columns={"new": "dead"})
    ...
    vaccine_doses = pd.read_csv(
        VACCINE_DOSES_CSV_PATH, index_col=["date"], usecols=["date", "new_doses"]
    ).rename(columns={"new_doses": "vaccine_doses"})
...
def get_timeseries_total():
    ...
    tested = pd.read_csv(
        TESTED_CSV_PATH, index_col=["date"], usecols=["date", "total"]
    ).rename(columns={"total": "tested"})

    confirmed = pd.read_csv(
        CONFIRMED_CSV_PATH, index_col=["date"], usecols=["date", "total"]
    ).rename(columns={"total": "confirmed"})

    dead = pd.read_csv(
        DEAD_CSV_PATH, index_col=["date"], usecols=["date", "total"]
    ...
    vaccine_doses = pd.read_csv(
        VACCINE_DOSES_CSV_PATH, index_col=["date"], usecols=["date", "total_doses"]
    ).rename(columns={"total_doses": "vaccine_doses"})
